...
public class Window extends PApplet {
  ...
  public void drawCursor(float diameter) {
  ...
  /**
   * Method for when a mouse is clicked or pressed.
   */
  public void mousePressed() {
    mouse.changePlayerDirection();
    for (AbstractCharacter f : characters) {
      if (mouse.getBoundingBox(this).collides(f.getBoundingBox()) && f instanceof FoodCharacter) {
        removeFood((FoodCharacter) f);
      }
    }
  }
   ...
   *
   * @param fd the food to add (and watch the JVM blow up on)
   */
  public void addNotThreadSafeFood(FoodCharacter fd) {
    if (this.currentNumberFood < maxFood) {
      this.currentNumberFood += 1;
      this.drawables.add(fd);
      this.characters.add(fd);
      this.collidables.add(fd);
    }
  }

  public void addFood(FoodCharacter fd) {
    addFoodQueue.add(fd);
  }
