...
import org.bcit.comp2522.labs.lab04.CharacterOutOfBoundsException;
...
public abstract class AbstractCharacter implements IDrawable, ICollidable {
  ...
  public void bounce(float a) {
    ...
    float dirX = getDirection().x;
    float dirY = getDirection().y;
    int dirx = Math.round(dirX);
    int diry = Math.round(dirY);

    this.setDirection(new PVector(dirx, diry));
  }
  ...
  public boolean touchWall(Window window) {
    if (getBoundingBox().getTop() <= 0
            || getBoundingBox().getBottom() >= window.height
            || getBoundingBox().getRight() >= window.width
            || getBoundingBox().getLeft() <= 0) {
      return true;
    } else {
      return false;
    }
