...
public class Window extends PApplet {
  ...
  /**
   * Called on mouse buttone pressed.
   */
  @Override
  public void mousePressed() {

    PVector mousePos = mouse.getPosition().copy();

    for (AbstractCharacter c : characters) {
      if (c instanceof FoodCharacter) {
        PVector foodPos = new PVector(c.position.x, c.position.y);

        if ((foodPos.x > mousePos.x - mouse.getDiameter()) && (foodPos.x < mousePos.x + mouse.getDiameter())
            && (foodPos.y > mousePos.y - mouse.getDiameter()) && (foodPos.y < mousePos.y + mouse.getDiameter())) {
          removeFood((FoodCharacter) c);
        }
      }
      player.setDirection(mouse.getPosition().copy().add(player.getPosition().copy().mult(-1f)).normalize());
    }
  }
